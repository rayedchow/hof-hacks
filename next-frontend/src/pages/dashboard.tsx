...
const Dashboard = () => {
  ...
  // Format date and time - only run when component is mounted (client-side)
  const dateOptions: Intl.DateTimeFormatOptions = { day: 'numeric', month: 'long' };
  const formattedDate = isMounted && currentTime ? currentTime.toLocaleDateString('en-US', dateOptions) : '';
  const formattedTime = isMounted && currentTime ? currentTime.toLocaleTimeString('en-US', { 
    hour: '2-digit', 
    minute: '2-digit', 
    second: '2-digit',
    hour12: true 
